...
**Acción acordada:** mover (no solo desactivar) los dos `.plist` fuera de `~/Library/LaunchAgents/` con `launchctl bootout` + `mv` a una carpeta de respaldo en el Desktop, para que no puedan recargarse solos ni siquiera después de un reinicio. El código de V1 se queda intacto en `~/chapultepec-bot` por si algún día se necesita revisar, solo se quita el mecanismo de auto-arranque. Confirmar en la próxima sesión que Carlos lo hizo y que las 3 llaves expuestas ya fueron rotadas.
